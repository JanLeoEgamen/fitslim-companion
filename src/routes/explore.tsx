...
import { EXPLORE_TOPICS, TOPIC_DETAIL } from "@/lib/fitslim/data";
      ...
      <PageLayout>
              ...
              {detail.questions.map((q) => (
                <li key={q}>
                  <AskAiButton
                    prompt={q}
                    label={q}
                    className="h-auto w-full justify-start whitespace-normal rounded-[12px] bg-pale-teal px-3 py-2 text-left text-[13px] font-medium text-navy hover:bg-light-blue"
                  />
                </li>
              ))}
            </ul>
              ...
              {detail.prompts.map((p) => (
                <li key={p}>
                  <AskAiButton
                    prompt={p}
                    label={p}
                    className="h-auto w-full justify-start whitespace-normal rounded-[12px] bg-light-blue px-3 py-2 text-left text-[13px] font-medium text-navy hover:bg-pale-teal"
                  />
                </li>
              ))}
            </ul>
        ...
        <div className="mt-5 flex items-center gap-3 rounded-[20px] border border-teal/20 bg-pale-teal p-4">
          ...
          <p className="text-sm text-navy">
            Save this topic's ideas to your{" "}
            <Link to="/saved" className="font-semibold underline underline-offset-2">
              Saved
            </Link>{" "}
        ...
        {EXPLORE_TOPICS.map((item) => (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="font-display block text-[16px] font-bold text-navy">
                {item.title}
              </span>
